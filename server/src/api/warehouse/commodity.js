...
import { createCommoditySchema, updateCommoditySchema } from "../../schema/commodity.js";
...
route.post("/create", validBody(createCommoditySchema, "商品信息不正确!"), async (req, res, next) => {
  // 建立新的商品

  const { barcode, name, category_name, unit, size, in_price, sale_price, vip_points, is_delete, supplier_name } = req.body;
...
route.put("/update", validBody(updateCommoditySchema, "提交的商品信息不正确或不完整!"), async (req, res, next) => {
  // 更新商品信息

  const { current_barcode, update_value } = req.body;
  const { barcode, name, category_name, unit, size, in_price, sale_price, vip_points, is_delete, supplier_name } = update_value;
