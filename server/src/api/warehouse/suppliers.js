...
import { createSupplierSchema, updateSupplierSchema } from "../../schema/suppliers.js";
...
route.post("/create", validBody(createSupplierSchema, "请输入正确的供应商信息!"), async (req, res, next) => {
  // 建立新的供应商信息

  const { name, phone, phone2, description } = req.body;
  ...
  await SuppliersTask.createSupplier(name, phone, phone2, description);
...
route.put("/update", validBody(updateSupplierSchema, "请输入正确的供货商信息!"), async (req, res, next) => {
  // 修改供货商信息
  ...
  const { new_name, new_phone, new_phone2, new_description } = update_value;
