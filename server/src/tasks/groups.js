...
class GroupTask {
  ...
  static async getGroupAuthority(group_id, onlyID = false) {
    ...
    return await Promise.all(authority_id_list.map(async ({ authority_id }) => await AuthorityTask.getAuthorityDetails(authority_id)));
    // 返回所有权限的完整信息
  }
  ...
  static async createGroup(group, authorityIDList) {
    ...
    await Promise.all(authorityIDList.map((id) => this.setGroupAuthority(lastID, id)));
  }
