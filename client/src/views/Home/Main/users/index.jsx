...
export function UserManage() {
  ...
  async function addGroup(usergroup, cb) {
    try {
      await Group.addGroup(ajax, usergroup);
      cb && cb();
      message.success("用户组添加完成!");
    } catch (error) {
      console.log(error);
    }
  }
  ...
  return (
    <div className={styled["user-manage-wrap"]}>
      <UserManageList {...usersData} list={list} selectId={selectId} selectType={selectType} handleClickSelect={handleClickSelect} />
      <UserManageFn
        ajax={ajax}
        groups={groups}
