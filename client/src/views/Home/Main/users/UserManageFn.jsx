...
function _ChangeModal({ type, status, form, closeModal, groups, user, createUser, changeUsername, changeUserPwd, changeUserGroup, addGroup }) {
  ...
  const { getFieldDecorator, getFieldsError, getFieldsValue, getFieldValue, isFieldsTouched, isFieldTouched } = form;
  ...
  function handleChangeGroup() {
    ...
    changeUserGroup(name, group, closeModal);
  }
  ...
  return (
    ...
    >
      <Form {...formLayout}>
        {fields.map(({ id, label, rules = [], ComponentFn, disabled, type: _type = "text" }) => (
          <Form.Item label={label} key={label}>
            ...
            })(ComponentFn ? ComponentFn(disabled[type]) : <Input type={_type} disabled={disabled[type]} />)}
          </Form.Item>
        ))}
      </Form>
    </Modal>
...
export function UserManageFn({ groups, user, ajax, createUser, changeUsername, changeUserPwd, changeUserGroup, addGroup, changeUserStatus }) {
  ...
  const config = [
    ...
    },
    {
      title: "新增用户组",
      icon: "user-add",
      fn: setType.bind(null, "group-add"),
    },
    {
      title: "修改用户组",
      icon: "edit",
  ...
  return (
    <div className={styled["user-manage-fn"]}>
      {config.map(({ title, btnType = "primary", icon, disabled = false, fn }) => (
        <Button type={btnType} key={title} icon={icon} disabled={disabled} onClick={fn}>
          {title}
        </Button>
      ))}
      <ChangeModal
