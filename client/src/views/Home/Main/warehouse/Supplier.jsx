...
function SupplierSearch({ btnList, isDefault }) {
  ...
  return (
    <div className={styled["supplier-search"]}>
      <Search value={value} ref={inputRef} onKeyDown={handleKeyDown} onChange={handleInput} placeholder="请在此输入需要查找的供应商信息" />
      {btnList
        .filter(({ show = true }) => show)
        .map(({ title, key, fn, type, icon = "", defaultCheck = false }) => (
          <Button icon={icon} type={type} key={key} onClick={fn} disabled={defaultCheck ? isDefault : false}>
            {`${title} <${key}>`}
          </Button>
        ))}
    </div>
...
export function Supplier() {
  ...
  return (
    <div className={styled["supplier-wrap"]}>
      ...
      <SupplierList list={supplierList} select={select} selectType={selectType} handleClick={handleClickSelect} />
      <SupplierModal flushList={getSupplierList} status={modalData.status} type={modalData.type} hideFn={hideModal} supplier={supplier} />
    </div>
  );
}
...
function SupplierList({ list, select, selectType, handleClick }) {
  ...
  return (
    <VirtualSelectList wrapCss={styled["supplier-list"]} data={list} select={select} selectType={selectType} columns={SupplierListColumns} footerColumn={footerData} handleClickSelect={handleClick} />
  );
}
...
function _SupplierModal({ hideFn, status = false, type = "增加", form, flushList, supplier }) {
  ...
  async function addSupplier() {
    const { name, phone, phone2, comment } = getFieldsValue();
    try {
      await SupplierTask.createSupplier(ajax, name, phone, phone2, comment);
      await flushList();
      hideFn();
  ...
  async function editSupplier() {
    const { name: new_name, phone: new_phone, phone2: new_phone2, comment: new_description } = getFieldsValue();
    const { name, phone, phone2, description } = supplier;
    let data = {};
    if (name !== new_name) {
      data = {
        ...data,
    ...
    if (phone2 !== new_phone2 && new_phone2 !== "") {
      data = {
        ...data,
        new_phone2,
      };
  ...
  const formConfig = [
    ...
    },
    {
      ...
      placeholder: "请在此输入供应商联系方式",
      config: {
        initialValue: isAddFlag ? "" : supplier.phone2 || "",
        rules: [
          {
            min: 5,
            max: 11,
      ...
      <Form {...formItemLayout}>
        {formConfig.map(({ label, field, placeholder, config }) => (
          <Item label={label} key={field}>
            {getFieldDecorator(field, config)(field === "name" ? <Input placeholder={placeholder} ref={inputRef} /> : <Input placeholder={placeholder} />)}
          </Item>
        ))}
      </Form>
    </Modal>
