...
export function _BaseForm({ form, selectCommodity, initState, type, closeDrawer, flushFn, updateFn }) {
  ...
  const CategorySelectTree = <CustomSelectTree tree={tree} value={categoryValue} onChange={setCategoryValue} placeholder="选择分类" />;

  const DeleteToggle = <Switch onChange={setIsDelete} checked={isDelete} />;

  const VipPointsToggle = <Switch onChange={setIsVipPoints} checked={isVipPoints} />;
    ...
    },
    {
      children: [
        ...
        },
        {
          ...
          label: "供货商",
          id: "supplier",
          component: useMemo(() => <SupplierSelect value={supplierValue} handleChange={setSupplierValue} ajax={ajax} />, [supplierValue]),
        },
      ],
    },
    {
  ...
  async function handleCreateCommodity() {
    ...
    if (newCommodity.barcode === "") {
      delete newCommodity.barcode;
    }
...
export function CommodityForm({ status = false, type, hideFn, selectCommodity, categories, flushFn, updateFn }) {
  ...
  return (
    <Drawer visible={status} title={title} width={450} onClose={closeDrawer} destroyOnClose>
      <BaseForm selectCommodity={selectCommodity} type={type} closeDrawer={closeDrawer} initState={initState} categories={categories} flushFn={flushFn} updateFn={updateFn} />
    </Drawer>
  );
}
